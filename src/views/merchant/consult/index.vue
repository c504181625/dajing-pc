...
const searchFields = [
  { label: '关键词', prop: 'keyword', placeholder: '咨询标题/企业名称/联系人' },
  {
    label: '状态',
    prop: 'status',
...
<template>
  <PageContainer
    :title="isDemander ? '我的咨询' : '咨询管理'"
    :subtitle="isDemander ? '查看本企业咨询记录与回复情况。' : '查看企业咨询并及时回复。'"
  >
      ...
      <el-table v-loading="loading" :data="tableData" border>
        ...
        <el-table-column label="操作" min-width="160" align="center" header-align="center">
          <template #default="{ row }">
            ...
            <PermissionButton
              v-if="!isDemander"
              permission="enterprise:consult:handle"
              text
    ...
    <el-drawer v-model="detailVisible" title="咨询详情" size="720px">
      <template v-if="currentDetail">
        ...
        <div class="drawer-actions" v-if="!isDemander">
          <PermissionButton permission="enterprise:consult:handle" @click="replyVisible = true"
            >咨询回复</PermissionButton
          >
        </div>
      </template>
    </el-drawer>
