...
async function submitInvalidate() {
  ...
  await invalidateReport(detail.value.id, invalidateReason.value)
...
async function handleToggleHidden() {
  ...
  ElMessage.success(detail.value.hidden ? '已取消隐藏' : '已隐藏')
}
...
<template>
  <PageContainer :title="detail?.reportNo || '报告详情'" subtitle="报告详情承载预览、下载、作废、隐藏和抽查记录。">
    <template #extra>
      <el-button @click="router.back()">返回列表</el-button>
      <StatusTag v-if="detail" :status="detail.status" :map="REPORT_STATUS_MAP" />
    </template>
    ...
    <el-row v-if="detail" :gutter="16">
      <el-col :span="16">
        <DetailSection title="报告基础信息">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item label="隐藏状态">{{ detail.hidden ? '已隐藏' : '可见' }}</el-descriptions-item>
          </el-descriptions>
        </DetailSection>
      ...
      <el-col :span="8">
        <DetailSection title="操作区">
          <div class="action-list">
            <el-button type="primary" @click="openPreview">预览</el-button>
            <el-link :href="detail.reportFile.url" target="_blank" type="primary">下载</el-link>
            <PermissionButton permission="report:manage:view" type="danger" plain @click="invalidateVisible = true">
              作废
            </PermissionButton>
            <PermissionButton permission="report:manage:view" plain @click="handleToggleHidden">
    ...
    <el-dialog v-model="invalidateVisible" title="作废报告" width="520px">
      <el-input v-model="invalidateReason" type="textarea" :rows="5" placeholder="请输入作废原因" />
      <template #footer>
