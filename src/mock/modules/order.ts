...
const orderRecords: OrderRecord[] = [
  {
    ...
    sampleItems: [
      { id: 'sample-001', sampleName: '芯片模组 A1', sampleCode: 'SM-A1-202604', sampleCount: 6, sampleRemark: '防潮包装' },
    ],
    projectItems: [
    ...
    attachments: [{ id: 'order-file-001', name: '寄样清单.xlsx', url: '/mock/sample-list.xlsx', fileType: 'xlsx' }],
    timeline: [
      {
        id: 'timeline-001',
        title: '委托已提交',
    ...
    reportInfo: {
      ...
      reportFile: { id: 'report-file-001', name: '检测报告.pdf', url: '/mock/report-001.pdf', fileType: 'pdf' },
    },
    commentInfo: {
      score: 5,
  ...
  },
  {
    ...
    sampleItems: [{ id: 'sample-002', sampleName: '量具样件', sampleCode: 'JG-20260407', sampleCount: 2 }],
    projectItems: [{ id: 'project-003', projectName: '计量校准', standardName: 'JJF 1001', price: 9800 }],
    attachments: [],
    timeline: [
      {
        id: 'timeline-101',
...
export function mockGetOrderList(params?: Record<string, unknown>) {
  ...
  if (keyword) {
    list = list.filter((item) => [item.orderNo, item.enterpriseName, item.orgName, item.projectName].some((field) => field.includes(keyword)))
  }
